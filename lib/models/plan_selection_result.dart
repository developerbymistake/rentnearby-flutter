/// Outcome of the shared GoLivePlanSheet (`lib/widgets/go_live_plan_sheet.dart`)
/// used identically by my_listings_screen.dart and my_plots_screen.dart before
/// a paid Go-Live. A dedicated sealed type — not an overloaded Map key on the
/// plan payload itself — so "user wants to top up first" can never be
/// confused with (or collide with a field name inside) an actual
/// selected-plan Map.
sealed class PlanSelectionResult {}

/// User picked an affordable plan and explicitly confirmed the spend (see the
/// "Confirm Spend" step folded into GoLivePlanSheet). [plan] is the raw plan
/// Map as returned by `GET /listings/plans` or `/plots/plans`.
