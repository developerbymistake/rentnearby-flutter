...
class _ExplorePlotsScreenState extends State<ExplorePlotsScreen>
  ...
  void initState() {
    ...
    // nearestActive/nearestFocusIndex live on PlotController (a session-lifetime
    ...
    _nearestActiveWorker = ever(_plotCtrl.nearestActive, (bool active) {
      if (!mounted) return;
      if (active) {
        _updateNearestFocus();
        _playTing();
    ...
    _nearestFocusWorker = ever(_plotCtrl.nearestFocusIndex, (_) {
      if (!mounted) return;
  ...
  void _buildMarkers({bool animate = true}) {
    ...
    if (_plotCtrl.nearestActive.value) {
      final nearest = _plotCtrl.nearestPlots;
      final idx = _plotCtrl.nearestFocusIndex.value;
      if (idx >= 0 && idx < nearest.length) {
        final focused = nearest[idx];
        final areaText = focused.areaDisplay;
        data.add(_MapMarkerData(
          position: LatLng(focused.latitude, focused.longitude),
          width: (areaText.length * 8.5 + 28).clamp(60.0, 110.0) + 10,
          height: 38,
          widget: GestureDetector(
            onTap: () => Get.toNamed(AppRoutes.plotDetail,
                      ...
                      children: [
                        ...
                        if (_filteredPlots.isEmpty && !_loadingNearby && !_reloadPending && _hasLoadedOnce)
                          Obx(() {
                            ...
                            if (_plotCtrl.nearestActive.value) return const SizedBox.shrink();
                ...
                return Positioned(
                  ...
                  child: Padding(
                    ...
                    child: NearestControlBar(
                      ...
                      total: total,
                      onPrev: () {
                        if (idx > 0) _plotCtrl.nearestFocusIndex.value = idx - 1;
                      },
                      onNext: () {
  ...
  // ── Nearest-N carousel preview card — compact version of the plot bottom
  // sheet's thumbnail/type/area, showing whichever plot is currently focused. ─
